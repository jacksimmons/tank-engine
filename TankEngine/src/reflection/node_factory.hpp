...
namespace Tank::Reflect
{
	...
	private:
		...
		std::optional<constructor_t> tryGetClass(std::string const &name) const
		{
			name_to_class_t::const_iterator it = m_nameToClassMap.find(name);
	...
	public:
		...
		Node *deserialise(const json &serialised) const;
	};
}